...
- 按 URL 去重，保留签名查询参数，并剔除可识别的广播和景区慢直播，避免混入地方台。默认每轮最多检测 **600 个候选URL**，优先自定义及上一轮通过的源，上游临时不可达时会把该上游上次通过的URL加入本轮重新检测，绝不直接作为已验证结果；其余候选按日期轮换，地方台约占发现名额的三分之二。不是每天穷举整个互联网；可以调整上限。
...
修改 config/settings.json 可设置检测上限、并发数、样本长度、单次FFmpeg超时和重试。默认并发10，同一主机最多2路同时检测，避免集中请求。脚本仅依赖 Python 3.10+ 标准库及 FFmpeg。中文 OCR 需要 Tesseract 的 chi_sim 和 eng 语言包。
